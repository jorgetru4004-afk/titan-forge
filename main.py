...
# ═══ STRATEGY CONFIG (swap after research data) ═══
ENABLED_STRATEGIES = [
    "MEAN_REVERT", "STOCH_REVERSAL", "VWAP_REVERT",
    "EMA_BOUNCE", "MOMENTUM_CONT", "BREAKOUT", "VWAP_TREND",
    "LONDON_BREAKOUT", "ORB", "VOL_SQUEEZE",
    "LIQUIDITY_SWEEP", "PREV_DAY_BOUNCE", "PREV_DAY_HL",
]
ENABLE_SCALPS = True
ENABLE_DIRECTION_ENGINE = True
COOLDOWN_SECONDS = 90
SL_ATR_MULT = 1.0    # Default SL = 1.0 × ATR (swap from research)
TP_ATR_MULT = 1.5    # Default TP = 1.5 × ATR (swap from research)
...
class API:
    def __init__(self, token, acct_id):
        self.token = token
        self.acct = acct_id
        self.base = "https://mt-client-api-v1.agiliumtrade.agiliumtrade.ai"
        self.hdr = {"auth-token": token, "Content-Type": "application/json"}
        self.symbols: Dict[str, str] = {}

    def connect(self):
        try:
            r = req.get(f"https://mt-provisioning-api-v1.agiliumtrade.agiliumtrade.ai/"
                        f"users/current/accounts/{self.acct}",
                        headers={"auth-token": self.token}, timeout=15)
    ...
    def account_info(self):
        try:
            ...
            return r.json() if r.status_code == 200 else {"balance": 0, "equity": 0}
        except: return {"balance": 0, "equity": 0}
    ...
    def candles(self, symbol, tf="15m", count=300):
        try:
            r = req.get(f"{self.base}/users/current/accounts/{self.acct}/"
                        ...
                        headers=self.hdr, params={"limit": count}, timeout=20)
            if r.status_code == 200:
                cc = r.json()
                if cc and len(cc) >= 50:
                    return {
    ...
    def order(self, symbol, direction, lots, sl=0, tp=0, comment=""):
        try:
            ...
            r = req.post(f"{self.base}/users/current/accounts/{self.acct}/trade",
                         headers=self.hdr, json=payload, timeout=15)
            if r.status_code == 200:
    ...
    def close(self, pid):
        try:
            r = req.post(f"{self.base}/users/current/accounts/{self.acct}/trade",
                         headers=self.hdr,
                         json={"actionType": "POSITION_CLOSE_ID", "positionId": pid}, timeout=15)
    ...
    def resolve_symbols(self):
        for name, mt5s in INSTRUMENTS.items():
            d = self.candles(mt5s, "15m", 1)
            if d:
                self.symbols[name] = mt5s
                logger.info(f"  ✅ {name} → {mt5s}")
            else:
                d = self.candles(name, "15m", 1)
                if d:
...
def main():
    ...
    logger.info("║  14 Strats | Direction | Scalps | Gate | Health         ║")
    logger.info("╚══════════════════════════════════════════════════════════╝")
                ...
                for sym, d in cdata.items():
                    try:
                        ...
                        for s in sigs:
                            ...
                            # Risk budget
                            rdol = s["atr"] * DOLLAR_PER_UNIT.get(sym, 100000) * SL_ATR_MULT
                            if daily_gate:
                            ...
                            av = s["atr"]; p = s["price"]
                            if s["dir"] == 1:
                                sl = p - SL_ATR_MULT * av; tp = p + TP_ATR_MULT * av
                            else:
                                sl = p + SL_ATR_MULT * av; tp = p - TP_ATR_MULT * av
                            # Execute
