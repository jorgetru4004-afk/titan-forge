...
class API:
    ...
    async def candles(self, symbol, tf="15m", count=300):
        """Get candles - try multiple methods"""
        from datetime import timedelta
        import urllib3
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
        
        # Method 1: SDK historical market data API
        try:
            start = datetime.now(timezone.utc) - timedelta(hours=count * 0.25 + 24)
            cc = await self.meta_api.historical_market_data_api.get_historical_candles(
                self.acct, self.account.region, symbol, tf, start, count
            )
            if cc and len(cc) >= 50:
                return self._parse_candles(cc)
        except Exception as e:
            logger.debug(f"[CANDLE-SDK] {symbol}: {e}")

        # Method 2: REST with market-data-specific URL
        try:
            region = getattr(self.account, 'region', 'london')
            for base_tpl in [
                f"https://mt-market-data-client-api-v1.{region}.agiliumtrade.agiliumtrade.ai",
                f"https://mt-client-api-v1.{region}.agiliumtrade.agiliumtrade.ai",
            ]:
                hdr = {"auth-token": self.token, "Content-Type": "application/json"}
                url = (f"{base_tpl}/users/current/accounts/{self.acct}/"
                       f"historical-market-data/symbols/{symbol}/timeframes/{tf}/candles")
                r = req.get(url, headers=hdr, params={"limit": count}, timeout=20, verify=False)
                if r.status_code == 200:
                    cc = r.json()
                    if cc and len(cc) >= 50:
                        return self._parse_candles(cc)
        except Exception as e:
            logger.debug(f"[CANDLE-REST] {symbol}: {e}")

        # Method 3: SDK RPC get_candle (only latest, build array from terminal state)
        try:
            candle = await self.connection.get_candle(symbol, tf)
            if candle:
                logger.info(f"[CANDLE] {symbol}: got single candle via RPC (need historical)")
        except Exception as e:
            logger.debug(f"[CANDLE-RPC] {symbol}: {e}")

        logger.warning(f"[CANDLE] {symbol}: all methods failed")
        return None
