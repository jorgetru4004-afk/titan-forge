...
class API:
    ...
    def candles(self, symbol, tf="15m", count=300):
        """Candles via REST — doesn't need SDK sync"""
        try:
            region = getattr(self.account, 'region', 'london') if self.account else 'london'
            base = f"https://mt-client-api-v1.{region}.agiliumtrade.agiliumtrade.ai"
            hdr = {"auth-token": self.token, "Content-Type": "application/json"}
            r = req.get(f"{base}/users/current/accounts/{self.acct}/"
                        f"historical-market-data/symbols/{symbol}/timeframes/{tf}/candles",
                        headers=hdr, params={"limit": count}, timeout=20, verify=False)
            if r.status_code == 200:
                cc = r.json()
                if cc and len(cc) >= 50:
                    return {
    ...
    def resolve_symbols(self):
        """Try to resolve symbols via REST candle check"""
        import urllib3
        ...
        for name, mt5s in INSTRUMENTS.items():
            for sym_try in [mt5s, name]:
                try:
                    r = req.get(f"{base}/users/current/accounts/{self.acct}/"
                                f"historical-market-data/symbols/{sym_try}/timeframes/15m/candles",
                                headers=hdr, params={"limit": 5}, timeout=15, verify=False)
