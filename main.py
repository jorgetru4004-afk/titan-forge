...
class API:
    def __init__(self, token, acct_id):
        self.token = token
        self.acct = acct_id
        self.base = ""  # Set dynamically in connect() based on region
    ...
    def connect(self):
        try:
            ...
            if r.status_code == 200:
                ...
                logger.info(f"[MT5] Account state: {state} | Region: {region}")
                # Set base URL based on actual region
                self.base = f"https://mt-client-api-v1.{region}.agiliumtrade.agiliumtrade.ai"
                self.hdr = {"auth-token": self.token, "Content-Type": "application/json"}
                logger.info(f"[MT5] Client API: {self.base}")
